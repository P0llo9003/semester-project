...
public class PagoController {
    ...
    public ResponseEntity<PagoDTO> crearPago(@Valid @RequestBody PagoDTO pagoDto) {
        try {
            logger.info("POST /pagos - Creando pago: idEntrada={}, totalPagar={}", 
                pagoDto.getIdEntrada(), pagoDto.getTotalPagar());
            Pago nuevoPago = pagoService.save(pagoDto.toModel());
    ...
    public ResponseEntity<PagoDTO> actualizarPago(@PathVariable Long id, @Valid @RequestBody PagoDTO pagoDto) {
        ...
        try {
            Pago actualizado = pagoService.update(id, pagoDto.toModel());
